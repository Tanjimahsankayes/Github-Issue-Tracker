...
async function loadIssues() {
  ...
  try {
    const response = await fetch(apiEndPoint.allIssues);
    if (!response.ok) {
...
function demonstrateArrayOperations(issues) {
  ...
  const searchResults = issues.filter(
    (issue) =>
...
async function handleSearch() {
  ...
  try {
    const response = await fetch(apiEndPoint.searchIssues(query));
    if (!response.ok) {
...
function createIssueCard(issue) {
  ...
  card.className =
    "issue-card bg-white rounded-lg shadow-sm border border-gray-200 cursor-pointer overflow-hidden";
  ...
  const labelsHTML = issue.labels
    ? issue.labels
        .map(
          (label) =>
            `<span class="inline-block bg-red-300 text-red-900 text-xs px-2 py-1 rounded-full">${label}</span>`,
        )
        .join(" ")
    : "";
        ...
        <div class="p-4">
            ...
            <div class=" flex item-end mb-3">
                ...
                <div class="mb-3">
                    ...
                    <p class="text-[12px] text-gray-500 line-clamp-3">${issue.description || "No description available"}</p>
...
async function openIssueModal(issue) {
  try {
    const response = await fetch(apiEndPoint.singleIssue(issue.id));
    if (!response.ok) {
    ...
    document.getElementById("modalPriority").className =
      `px-3 py-1 rounded-full text-sm font-medium ${getPriorityColor(fullIssue.priority)}`;
    document.getElementById("modalAuthor").textContent =
      fullIssue.author || "Unknown";
    ...
    document.getElementById("modalDescription").textContent =
      fullIssue.description || "No description available";
    ...
    if (fullIssue.labels && fullIssue.labels.length > 0) {
      fullIssue.labels.forEach((label) => {
        ...
        labelSpan.className =
          "inline-block bg-gray-100 text-gray-700 text-xs px-2 py-1 rounded";
        labelSpan.textContent = label;
        modalLabels.appendChild(labelSpan);
      });
    } else {
      modalLabels.innerHTML =
        '<span class="text-gray-500 text-sm">No labels</span>';
    }
...
function populateModalWithBasicData(issue) {
  ...
  document.getElementById("modalPriority").textContent =
    issue.priority || "MEDIUM";
  document.getElementById("modalPriority").className =
    `px-3 py-1 rounded-full text-sm font-medium ${getPriorityColor(issue.priority)}`;
  document.getElementById("modalAuthor").textContent =
